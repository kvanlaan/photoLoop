...
/**
 * A Loop ADT is a circular sequence of items. A Loop has a current item and the
 * ability to move forward or backwards. A Loop can be modified by removing the
 * current item or by adding an item before the current item.
 */
public interface LoopADT<E> {

	/**
	 * Adds the given item immediately <em>before</em> the current item. After
	 * the new item has been added, the new item becomes the current item.
	 * 
	 * @param item
	 *            the item to add
	 */
